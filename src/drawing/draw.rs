...
use log::{info, log};
use raylib::{
    camera::Camera3D, color::Color, ffi::{rlEnd, rlVertex3f}, math::{Vector2, Vector3}, prelude::{RaylibDraw, RaylibDraw3D, RaylibDrawHandle, RaylibMode3D, RaylibMode3DExt}
};

use crate::{drawing::rlgl::Raylib3DHandle, world::Obstacle, State};
...
pub fn draw(h: &mut RaylibDrawHandle<'_>, state: &State) {
    ...
    let _handle = h.begin_mode3D(state.display.camera);

    draw_world(&mut 3d_handle, &state);
    draw_ui(h, &state);
}
...
fn draw_world(h: &mut Raylib3DHandle, state: &State) {
  ...
  let points:Vec<Vector3> = state.world.navmesh.nodes
  .iter()
